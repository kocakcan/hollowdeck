# Art Credits

## Character & enemy sprites — Dungeon Crawl Stone Soup tiles (CC0 / public domain)

The 32×32 pixel sprites in `assets/sprites/` come from the
[Dungeon Crawl Stone Soup](https://github.com/crawl/crawl) tile set
(`crawl-ref/source/rltiles/mon/`), released under CC0.

They have since been **palette-clamped to the shared ramp** by
`tools/artgen clamp` (`docs/ART_SPEC.md` §5). The set was contributed by
different artists over roughly twenty years and carried anywhere from 7 to 379
distinct colours per sprite; clamping brings every one of them onto the same
43 entries, which is what makes them read as one game's art rather than as a
tile pack. CC0 imposes no conditions, so the modification needs no notice —
it is recorded here because it is the single largest change made to sourced
material in the project.

Mapping:

Act I:

| Game use | Source tile |
| --- | --- |
| Player avatar | `humanoids/humans/hell_knight.png` |
| Cultist | `humanoids/humans/occultist.png` |
| Acid Slime | `amorphous/slime_creature.png` |
| Rot Hound | `animals/hound.png` |
| Ward Acolyte | `vault/cloud_mage.png` |
| Bog Troll | `humanoids/troll.png` |
| Possessed Armor | `undead/ancient_champion.png` |
| Hollow King | `undead/ancient_lich.png` |
| The Gaol Warden | `humanoids/oni_incarcerator.png` |
| Drowned Thrall | `undead/drowned_soul.png` |
| Mire Leech | `animals/tyrant_leech.png` |
| Gaol Rat | `animals/rat.png` |
| The Drowned Matron | `undead/silent_spectre.png` |

Act II:

| Game use | Source tile |
| --- | --- |
| Ember Wisp | `nonliving/fire_vortex1.png` |
| Slag Brute | `nonliving/iron_golem.png` |
| Cinder Cultist | `vault/hellbinder.png` |
| Ash Stalker | `demons/smoke_demon.png` |
| Molten Sentinel | `nonliving/blazeheart_golem.png` |
| Pyre Warden | `demons/efreet.png` |
| Emberlord Vashk | `unique/asmodeus.png` |
| The Slag Maw | `amorphous/rockslime.png` |
| Cinder Moth | `animals/moth_of_wrath.png` |
| Ashen Priest | `undead/searing_wretch.png` |
| Forge Wight | `nonliving/molten_gargoyle.png` |
| The Emberforge Smith | `humanoids/ironbound_mechanist.png` |

Act III:

| Game use | Source tile |
| --- | --- |
| Hollow Shade | `undead/shadow_wraith.png` |
| Bone Choir | `undead/revenant.png` |
| Throne Sentry | `statues/obsidian_statue.png` |
| Void Leech | `aberrations/tentacled_monstrosity.png` |
| Crown Reaver | `vault/antique_champion.png` |
| The Silent Judge | `holy/daeva.png` |
| The Hollow Throne | `vault/zot_statue.png` |
| The Nameless Regent | `undead/ancient_lich.png` |
| Wailing Effigy | `statues/pillar_of_salt.png` |
| Gilded Husk | `undead/guardian_mummy.png` |
| Oath Binder | `undead/skeletal_warrior.png` |
| The Sable Inquisitor | `demons/reaper.png` |

## Backgrounds — generated, no sourced material

The nine floor tiles in `assets/backgrounds/` are generated by `tools/artgen`
(`src/icons/backgrounds.rs`) and are original work — three per act, on the
64×64 tile grid from `docs/ART_SPEC.md` §1, built from the same 43-colour ramp
and the same 45° lamp as the icons. No attribution is required.

### Retired

They were previously seven CC0 Dungeon Crawl dungeon floors from
`crawl-ref/source/rltiles/dngn/floor/` (`crypt0`, `black_cobalt01`, `dirt0`,
`etched0`, `cobble_blood1`, `floor_gulch0`, `demonic_red1`), upscaled 2× and
palette-clamped. Clamping put them on the ramp and a per-act `Modulate` made
them warmer or cooler, but neither could make them belong: they are top-down
rubble textures authored for a different game, and the floor is the largest
surface on screen to have disagreeing with everything drawn on it. Since no
Dungeon Crawl floor material ships any more, only the creature tiles below
carry the CC0 note.

## Icons — generated, no sourced material

All 161 icons in `assets/icons/` are generated by `tools/artgen` (see its
README) and are original work. Each one is a `fn` composing shapes from a
shared vocabulary onto a 32×32 grid, using only the 43-colour ramp in
`docs/ART_SPEC.md` §5. No attribution is required.

### Retired

The icons were previously SVGs from [game-icons.net](https://game-icons.net)
under CC BY 3.0, by Lorc, Delapouite, Skoll, sbed, Willdabeast, Andy Meneely
and Caro Asercion. They were removed when the project committed to pixel art
as a single medium: vector art is not pixel art, and downscaling it onto a
32×32 grid produces mush rather than a sprite. The generated set replaced all
78 of them one-for-one — plus a 79th (`map/event`) that the SVG set never had,
which is why Event nodes rendered as the word "EVENT" on the map. Since no
game-icons.net material ships any more, the CC BY attribution no longer needs
to travel with the build.

## Typography — Google Fonts (SIL Open Font License 1.1)

Fonts in `assets/fonts/` are used under the
[SIL OFL 1.1](https://openfontlicense.org/), which requires the license text
to ship alongside the font files (see the `*-OFL.txt` files next to each
font).

| Font | Copyright | Use |
| --- | --- | --- |
| [Silkscreen](https://fonts.google.com/specimen/Silkscreen) | © 2001 The Silkscreen Project Authors | Display face — titles, buttons, card/enemy names, HP/energy numbers, floating damage text |
| [Tiny5](https://fonts.google.com/specimen/Tiny5) | © 2022–2024 The Tiny5 Project Authors | Body face — card descriptions and general UI text |

Both are bitmap/pixel faces, imported with antialiasing, hinting and subpixel
positioning disabled so glyphs stay on the pixel grid — see
`docs/ART_SPEC.md` §7.

### Retired

Cinzel (Natanael Gama) and IM Fell English (Igino Marini) were the previous
display/body pair. They were dropped when the project committed to pixel art
as a single medium: high-res serif faces are anti-aliased sub-pixel curves,
so keeping them would have moved the art/text seam rather than closed it.
Their files have been removed from `assets/fonts/`.

Pixelify Sans was also trialled as the body face and rejected: at 16px its
digits `2`, `3`, `5` and `8` are mutually ambiguous, which is disqualifying
for a game whose text is mostly numbers. Its files were removed too.

Jersey 15 (The Soft Type Project Authors) was the body face after that, and
was replaced by Tiny5 for the same reason arrived at from the other
direction: its design em is 27px — the "15" in its name is its cap height —
so at the 16px the UI rendered it, glyphs fell off the pixel grid and `Deal 6
damage` came out as `Deal 8 damage`. Silkscreen Regular went at the same
time; only the Bold weight was ever referenced. Both are removed from
`assets/fonts/`.

## UI chrome — procedural, no sourced material

All chrome is original and needs no attribution. It comes from two places:
the 9-slice border art behind panels, buttons, HUD slots and the art plinth
is generated by `tools/artgen`'s `chrome` category into `assets/theme/`, and
the boxes whose colours are decided at runtime — card frames, badges, HP bars
— are still built procedurally in `scripts/ui/ChromeStyles.cs` from the same
shared palette.

### Retired

The End Turn and main-menu buttons previously used ornate wooden frame
textures from [Fantasy UI Box](https://opengameart.org/content/fantasy-ui-box)
by **StumpyStrust** (CC0). They were removed when the project committed to
pixel art: the frames are smooth, anti-aliased fantasy art, and placing them
beside bitmap type and 32×32 sprites was exactly the mixed-media seam the
commitment exists to close.

The ornament they carried is back, in the medium it should have been in from
the start — the generated bezels above. That is a replacement rather than a
reinstatement: nothing sourced returned, and the retirement stands.

## Audio — procedurally generated, no sourced material

Every sound in the game (SFX and music) is synthesized in-engine at runtime
from raw oscillators/envelopes/filters (`scripts/audio/AudioSynth.cs`,
`AudioCues.cs`, `AudioMusic.cs`) - no samples, recordings, or third-party
audio assets were used. No attribution is required.
