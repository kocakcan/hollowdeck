...
public static class ArtAssets
{
    ...
    // A 9-slice chrome frame (docs/ART_SPEC.md section 6), generated by
    // artgen's `chrome` category into assets/theme/ beside the theme resource.
    // Named rather than keyed on a definition id - a panel is not content - so
    // this follows BackgroundTile's shape rather than CardIcon's.
    //
    // The names live in ChromeStyles.Slices, and PixelSpecSmokeTest walks that
    // list against the directory in both directions. That matters more here
    // than for any other lookup: a null texture makes a StyleBoxTexture draw
    // *nothing*, so a renamed slice would take a panel out of the interface
    // silently rather than degrading it to text the way missing icon art does.
    public static Texture2D? ChromeSlice(string name) => Load($"res://assets/theme/{name}.png");
