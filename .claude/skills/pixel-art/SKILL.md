...
every check the project has.** `impact`/`ward`/`bloom`/`venom`/`swipe` (`tools/artgen/src/icons/fx.rs`) are
five authored four-frame runs — the four bursts beat flash, ring, arms, motes — spawned by `scripts/ui/CombatFx.cs` and
...
- **`swipe` travels, and that is the whole of how a slash gets a direction.** `PlayTravelling` tweens
  `position` between two `SnapTranslation`'d endpoints while the frames run — legal because §9 bans
  transforms, not translations, and `PlayPlayerPositionBeat` already moves the player sprite this
