...
|---|---|---|
| `EffectSmokeTest` | pile + effect resolution, generated card/potion description text, rarity coverage *over the offerable pool*, `CardPool` weighting and its unplayable exclusion, `PotionPool`'s own 65/25/10 table plus the per-*row* monotonicity that content growth can silently invert, `RarityPool`'s shared algorithm (drains every tier once, fixed order, renormalises an exhausted tier, actually reads the weight function), every potion row *authoring* a rarity rather than defaulting to Common, Power routing, every playable card's `+` actually changing something, a negative `gain_gold` stealing and clamping at zero, no card/potion/relic authoring the enemy-only `summon_enemy`/`escape`, and the four Phase 8 statuses: `Artifact` refused-iff-`IsDebuff` over the whole enum, `Thorns` billing a fully-blocked attack but not `lose_hp`, `Intangible` flooring after Vulnerable, `Plating` granting and eroding only on damage that gets through, and turn-end decay reaching both combatants | `scripts/effects/`, `PileManager`, `CardPool`/`PotionPool`/`RarityPool`, `cards.json`, `potions.json`, `StatusType`, `StatusRow.IsDebuff`, `CombatManager.DecayAtTurnEnd` |
