...
|---|---|---|
| `EffectSmokeTest` | pile + effect resolution, generated card/potion description text, rarity coverage *over the offerable pool*, `CardPool` weighting and its unplayable exclusion, `PotionPool`'s own 65/25/10 table plus the per-*row* monotonicity that content growth can silently invert, `TierPool`'s shared algorithm (drains every tier once, fixed order, renormalises an exhausted tier, actually reads the weight function), every potion row *authoring* a rarity rather than defaulting to Common, Power routing, every playable card's `+` actually changing something, a negative `gain_gold` stealing and clamping at zero, no card/potion/relic authoring the enemy-only `summon_enemy`/`escape`, and the four Phase 8 statuses: `Artifact` refused-iff-`IsDebuff` over the whole enum, `Thorns` billing a fully-blocked attack but not `lose_hp`, `Intangible` flooring after Vulnerable, `Plating` granting and eroding only on damage that gets through, and turn-end decay reaching both combatants | `scripts/effects/`, `PileManager`, `CardPool`/`PotionPool`/`RelicPool`/`TierPool`, `cards.json`, `potions.json`, `StatusType`, `StatusRow.IsDebuff`, `CombatManager.DecayAtTurnEnd` |
...
| `ScreenSmokeTest` | Reward/Shop/Treasure/Rest load, populate and show their art; the reward *list* — a row per offered reward and none for a reward not offered, gold and the relic granted only on the claim rather than before the screen loads, a claim that cannot be repeated, the potion row refused (but still shown) against a full belt, the card row opening the fan as a modal and a pick returning to the list instead of leaving the screen; the shop's card-removal picker opening, hiding the shop beneath it, and cancelling for free; the keyword panel tracking hover and focus independently | any non-combat screen, `ScreenChrome`, `RewardScreen`/`RewardContext`, `CardView`'s keyword tooltip, or its `.tscn` |
| `ActSmokeTest` | acts load, act progression, per-act content is distinct, no `summon_enemy` crossing an act, every act authoring both potion drop rates above zero and in range with the elite rate not below the normal one (a 0 disables drops for that act silently) | `acts.json`, `ActDefinition`, `RunState.AdvanceAct`, any `summon_enemy` spec |
...
| `BalanceSmokeTest` | the difficulty curve rising act over act, every elite and boss encounter costing what its node type promises (bands, plus no boss cheaper than the act's costliest elite, **and no normal reaching `BossCostLow`**), every escape move stealing more than the cheapest node it can flee, every enrage phase out-hitting its own normal phase, every `RunScore` threshold being reachable by some seed, the potion drop yield banded at both ends with a boss never rolling, upgrade amounts matching the documented formula | `enemies.json`, `acts.json`, `RunScore` thresholds, `CardUpgrade`, `MapGenerator` weights |
